...
def authorization_url(client_id: str, runame: str, state: str) -> str:
    if not valid_sandbox_client_id(client_id):
        raise OAuthError("invalid Sandbox Client ID configuration")
    if not valid_sandbox_runame(runame):
        raise OAuthError("invalid Sandbox RuName configuration")
    if not state:
        raise OAuthError("missing OAuth state")
    return AUTH_URL + "?" + urlencode({"client_id": client_id, "redirect_uri": runame, "response_type": "code", "scope": SELLER_SCOPE, "state": state}, quote_via=quote)
...
class EbayOAuthClient:
    ...
    async def exchange_code(self, code: str, runame: str) -> dict:
        response = await self.client.post(TOKEN_URL, data={"grant_type": "authorization_code", "code": code, "redirect_uri": runame}, headers={"Authorization": self._basic(), "Content-Type": "application/x-www-form-urlencoded"})
        if response.status_code >= 400:
            raise OAuthError(f"eBay token exchange failed ({response.status_code})")
        return response.json()

    async def refresh(self, refresh_token: str) -> dict:
        response = await self.client.post(TOKEN_URL, data={"grant_type": "refresh_token", "refresh_token": refresh_token, "scope": SELLER_SCOPE}, headers={"Authorization": self._basic(), "Content-Type": "application/x-www-form-urlencoded"})
        if response.status_code >= 400:
            raise OAuthError(f"eBay token refresh failed ({response.status_code})")
        return response.json()

    async def application_token(self) -> dict:
        response = await self.client.post(TOKEN_URL, data={"grant_type": "client_credentials", "scope": MARKET_SCOPE}, headers={"Authorization": self._basic(), "Content-Type": "application/x-www-form-urlencoded"})
        if response.status_code >= 400:
            raise OAuthError(f"eBay application-token request failed ({response.status_code})")
        return response.json()


def store_user_tokens(session: Session, cipher: TokenCipher, payload: dict, *, now: datetime | None = None) -> EbayCredential:
    now = now or datetime.now(UTC)
